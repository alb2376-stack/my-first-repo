...
This repo puts the technique to work on `2d-explorations.html`, which runs two
independent p5 sketches on the same page:

1. **2D Primitives** (`js/2d-primitives.js`): static circle pattern, drawn once into `#canvas-container-1`
2. **2D Interactive** (`js/2d-interactive.js`): mouse-trail animation over a gradient, into `#canvas-container-2`

Open `2d-explorations.html` in your browser to see both at once.
