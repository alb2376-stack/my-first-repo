...
(function() { // Start an immediately invoked function expression (IIFE) to avoid global scope pollution
  ...
  d3.csv("my-steps.csv", d => ({ // Load CSV file and transform each row
    ...
    const color = d3.scaleLinear()
      .domain([0, 10000, 20000])
      .range(["#e91e63", "#ffffff", "#ff6600"])
      .interpolate(d3.interpolateRgb)
      .clamp(true); // Pink → white → orange
...
Value: ${formatValue(d.Value)}`); // Tooltip shows date and value
    ...
    const legendScale = d3.scaleLinear() // Create linear scale
      .domain([0, 20000]) // Scale from 0 to 20000
    ...
    const legendAxis = d3.axisBottom(legendScale) // Create bottom-oriented axis
      ...
      .tickValues([0, 5000, 10000, 15000, 20000]); // Set specific tick values to show the full range clearly
    ...
    gradient.selectAll("stop") // Select all stops (initially empty)
      ...
      .attr("stop-color", d => color(d3.scaleLinear().domain([0, 1]).range([0, 20000])(d))); // Set color using same scale as main chart
    ...
    legendSvg.append("text") // Add text element for legend title
      ...
      .text("Daily Steps (0 to 20000)"); // Set text content
